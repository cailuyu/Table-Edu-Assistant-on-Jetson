...
def chat_with_image(image, prompt, chat_history):
    # print(chat_history)
    ...
    system = {
                "role": "system",
                "content": [
                    {"type": "text", "text": "做一个全能的AI助手，根据图片解答问题，没有图片的话请直接回答问题，回答是主要要一步一步思考，回答要准确详尽,非英文题目请用中文回答！"},
                ],
            }
...
with gr.Blocks() as demo:
    ...
    crop_button.click(crop_image, inputs=im_preview, outputs=im)

    chat = chat_image_button.click(chat_with_image, inputs=[im, message, chat_history], outputs=[chat_history, chatbox])
    chat.then(clear_image,outputs=[im,message])
    
    chat_submit = message.submit(chat_with_image, inputs=[im, message, chat_history], outputs=[chat_history,chatbox])
